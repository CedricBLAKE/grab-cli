// GRAB config — src/config.ts
// Manages ~/.grab/grab.json

import { homedir } from 'os';
import { join } from 'path';
import { existsSync, mkdirSync, readFileSync, writeFileSync } from 'fs';

export const GRAB_DIR = join(homedir(), '.grab');
export const CONFIG_PATH = join(GRAB_DIR, 'grab.json');
export const CREDENTIALS_PATH = join(GRAB_DIR, 'credentials');
export const WORKSPACE_PATH = join(GRAB_DIR, 'workspace');
...
export function ensureGrocDir(): void {
  if (!existsSync(GRAB_DIR)) mkdirSync(GRAB_DIR, { recursive: true });
...
export function loadConfig(): GrabConfig {
  ensureGrocDir();
...
export function saveConfig(config: GrabConfig): void {
  ensureGrocDir();
